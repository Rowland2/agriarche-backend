...
def calculate_metrics(df, commodity, month, years=None, market="All Markets"):
    ...
    # --- Price per kg ---
    price_col = 'price of bag' if 'price of bag' in df.columns else 'price'
    weight_col = 'kg per bag'

    df[price_col] = pd.to_numeric(df[price_col], errors='coerce')
    df[weight_col] = pd.to_numeric(df[weight_col], errors='coerce')

    df = df[df[weight_col] > 0]
    df['price_per_kg'] = df[price_col] / df[weight_col]
    ...
    if market != "All Markets" and 'market' in df.columns:
        filtered = filtered[filtered['market'] == market]

    filtered = filtered.dropna(subset=['price_per_kg']).sort_values('day')

    if filtered.empty:
        return {"metrics": {"avg": 0, "max": 0, "min": 0}, "chart_data": []}

    return {
        "metrics": {
            "avg": round(filtered['price_per_kg'].mean(), 2),
            "max": round(filtered['price_per_kg'].max(), 2),
            "min": round(filtered['price_per_kg'].min(), 2),
        },
        "chart_data": filtered[['day', 'price_per_kg', 'year']]
            .rename(columns={'price_per_kg': 'price'})
            .to_dict(orient='records')
    }

# ... keep your normalize_name and COMMODITY_INFO functions below ...
